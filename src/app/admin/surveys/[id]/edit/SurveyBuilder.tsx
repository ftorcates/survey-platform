...
export default function SurveyBuilder({ survey }: { survey: SurveyData }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div key={q.id} className="card" style={{ padding: '1.5rem', borderLeft: editingQuestionId === q.id ? '4px solid var(--color-primary)' : '1px solid var(--color-border)' }}>
            ...
            {editingQuestionId !== q.id ? (
              <>
                ...
                {survey.type !== 'FIXED_SCALE' && q.type !== 'TEXT' && (
                  <div style={{ marginLeft: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {q.options.map((opt: any) => (
                      <div key={opt.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.9rem', backgroundColor: 'rgba(255,255,255,0.56)', borderRadius: '1rem', border: '1px solid var(--color-border)' }}>
                ...
                {survey.type !== 'FIXED_SCALE' && q.type === 'TEXT' && (
                  <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'rgba(255,255,255,0.56)', padding: '1rem', borderRadius: '1rem', border: '1px solid var(--color-border)' }}>
                    <GitBranch size={16} color="var(--color-text-muted)" />
      ...
      {/* Add Question Box */}
      <div className="card" style={{ padding: '2rem', border: '2px dashed rgba(15, 118, 110, 0.24)', backgroundColor: 'rgba(255, 255, 255, 0.56)' }}>
